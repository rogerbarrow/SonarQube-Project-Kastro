pipeline {
    ...
    environment {
        ...
        DOCKERHUB_USERNAME = 'rbarrow612'  // Set this directly to your Docker Hub username
    ...
    stages {
        stage('Git Checkout') {
            steps {
                git branch: 'main', url: 'https://github.com/rogerbarrow/SonarQube-Project-Kastro.git'
            }
        }
        ...
        stage('Sonar Analysis') {
            steps {
                withSonarQubeEnv('sonar-server') {
                    sh "$SCANNER_HOME/bin/sonar-scanner -Dsonar.projectName=Roger -Dsonar.projectKey=RogerKey -Dsonar.java.binaries=target"
                }
            }
        }
